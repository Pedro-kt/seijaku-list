...
/**
 * UseCase para obtener manga que están actualmente en publicación usando AniList API
 *
 * @param repository Repositorio de AniList
 */
class GetPublishingMangaAniListUseCase @Inject constructor(
    private val repository: AnimeAniListRepository
) {
    /**
     * Obtiene manga que están siendo publicados actualmente
     *
     * @param page Número de página (default: 1)
     * @param perPage Elementos por página (default: 20)
     * @return Lista de AnimeCard de manga en publicación
     */
    suspend operator fun invoke(page: Int = 1, perPage: Int = 20): List<AnimeCard> {
        return repository.getPublishingManga(page = page, perPage = perPage)
    }
}
