...
/**
 * UseCase para obtener anime en tendencia usando AniList API
 *
 * @param repository Repositorio de AniList
 */
class GetTrendingAnimeAniListUseCase @Inject constructor(
    private val repository: AnimeAniListRepository
) {
    /**
     * Obtiene anime que están en tendencia actualmente
     *
    ...
    suspend operator fun invoke(page: Int = 1, perPage: Int = 20): List<AnimeCard> {
        return repository.getTrendingAnime(page = page, perPage = perPage)
    }
}
